...
一个交互式中文学习实验室：先用 ReAct、ACI、Minimal Agent、源码拆解与闭环评测建立通用 AI Agent 学习方法，再以 PI v0.84.4 的真实 API 和《动手学 Pi》course-v1 作为实现案例。通过 18 个模块、108 个可复现实验，从 Agent 基础一路搭建到 Todos 风格的本地多 Agent 协作平台。
...
14. Scout → Planner → Worker → Reviewer 多 Agent 协作
15. DAG、租约、并发、重试与背压调度
16. Extension + Skill + Prompt + Theme 插件包
17. Project Trust、容器、模型路由、成本与可观测性
18. Chief、任务 DAG、Git Worktree 与质量门禁
...
- [PI v0.84.4 Subagent 示例](https://github.com/earendil-works/pi/tree/v0.84.4/packages/coding-agent/examples/extensions/subagent)
